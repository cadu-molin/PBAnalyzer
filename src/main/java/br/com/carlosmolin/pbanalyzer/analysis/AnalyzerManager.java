package br.com.carlosmolin.pbanalyzer.analysis;
...
public class AnalyzerManager implements ParseTreeListener {
    ...
    private final List<WindowAnalyzer> analyzers = new ArrayList<>();
    ...
    public void addAnalyzer(WindowAnalyzer a) { analyzers.add(a); }
    ...
    private void invokeSpecific(ParserRuleContext ctx, String prefix) {
        ...
        for (WindowAnalyzer a : analyzers) {
    ...
    public List<Report> collectReports() {
        ...
        for (WindowAnalyzer analyzer : analyzers) {
            if (analyzer.getReport().getEntries().isEmpty()) continue;
